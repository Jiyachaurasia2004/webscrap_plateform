...
const Dashboard = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      setError(
        error.response?.data?.message || "Unable to search. Please try again.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleScrape = async (url) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setScrapeMessage("Website scraped and saved successfully!");
      }
    } catch (error) {
      console.error(error);
  ...
  const fetchStats = async () => {
    try {
      const [historyResponse, scrapedResponse] = await Promise.all([
        api.get("/history"),
        api.get("/scraper"),
      ]);

      setStats({
        totalSearches: historyResponse.data.count || 0,

        scrapedRecords: scrapedResponse.data.count || 0,

        searchHistory: historyResponse.data.count || 0,
  ...
  const handleSaveItem = async (result) => {
    try {
      ...
      setSaveMessage(error.response?.data?.message || "Unable to save item.");
    } finally {
      setSavingUrl("");
    }
  };
  ...
  return (
    <div className="h-screen bg-slate-100 flex overflow-hidden">
      ...
      <aside className="hidden md:flex w-64 h-screen shrink-0 bg-slate-950 text-white flex-col overflow-hidden">
        {/* Logo */}
        ...
        <div className="p-6 border-b border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-lg">WebScrape</h1>

              <p className="text-xs text-slate-400">Search Platform</p>
            </div>
          </div>
        </div>
    ...
    bg-blue-600 text-white font-medium"
          ...
          <button
            onClick={() => {
              document.getElementById("web-search")?.scrollIntoView({
                behavior: "smooth",
                block: "start",
              });
            }}
...
>
  ...
  Settings
      ...
      <main className="flex-1 min-w-0 h-screen overflow-y-auto">
        ...
        <header className="sticky top-0 z-30 bg-white border-b border-slate-200 px-5 md:px-8 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="hidden sm:flex w-10 h-10 rounded-full bg-blue-100 items-center justify-center">
              <User className="text-blue-600" size={19} />
            </div>
          </div>
        </header>
  ...
  rounded-3xl p-6 md:p-8 mb-8 shadow-lg scroll-mt-24"
          >
            <div className="max-w-4xl">
              ...
              <p className="text-blue-100 mt-2 mb-6">
                Search the web and discover useful public information from
                multiple sources.
              </p>
          ...
          {results.length > 0 && (
            ...
            >
              ...
              {saveMessage && (
                <div className="mb-5 bg-blue-50 border border-blue-200 text-blue-700 rounded-xl px-5 py-4">
                  {saveMessage}
                </div>
              )}
              {/* Results */}
    ...
    text-sm font-semibold transition"
                          ...
                          <button
                            ...
                            disabled={savingUrl === result.link}
                            className="inline-flex items-center gap-2
    ...
    text-sm font-semibold transition"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-slate-500">Scraped Records</p>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-slate-500">Recent Websites</p>
          ...
          >
            ...
            <div className="p-10 text-center">
              ...
              <h4 className="font-semibold text-slate-800">No activity yet</h4>

              <p className="text-sm text-slate-500 mt-2">
                Start by searching something from the search box above.
              </p>
            </div>
          </div>
        </section>
